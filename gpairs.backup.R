  ...
  if (!is.null(xpos)) {
    if (!is.null(cat.labels) && !horiz) {
      grid.text(cat.labels, x=unit(1:k, "native"),
                ...
                rot=outer.rot[2-xpos], gp=gpar(fontsize=axis.pars$fontsize))
  ...
  if (!is.null(ypos)) {
    if (!is.null(cat.labels) && horiz) {
      grid.text(cat.labels, y=unit(1:k, "native"),
                ...
                rot=outer.rot[2-ypos], gp=gpar(fontsize=axis.pars$fontsize))
    } else grid.yaxis(at=py, gp=gpar(fontsize=axis.pars$fontsize), main=ypos)
  }

} # End draw.axis()

qq.panel <- function(x, y, scatter.pars, axis.pars, xpos, ypos) {
  ...
  grid.rect()
  draw.axis(x, y, axis.pars, xpos, ypos, NULL, NULL)
  x <- sort(x)
  y <- sort(y)
...
scatterplot.panel <- function(x, y, type, scatter.pars, axis.pars, xpos, ypos) {
  ...
  if (type=="corrgram") { # in the style of Friendly (2002)
    ...
    panel.fill(col = hsv(h = zebra.col, s = abs(as.numeric(corr)), v = 1),
               border = hsv(h = zebra.col, s = abs(as.numeric(corr)), v = 1))
  ...
  if (type=="stats") {
    ...
    rho <- format(rho.test$estimate, digits = 2)
    tau <- format(tau.test$estimate, digits = 2)
    
    xy.lm <- lm(y ~ x)
    r2 <- format(summary(xy.lm)$r.squared, digits = 2)
    p <- format(pf(q=as.numeric(summary(xy.lm)$fstatistic)[1],
    ...
    if (!is.na(stat.pars$verbose)) {	
      if (stat.pars$verbose == TRUE) {
        ...
        grid.text(paste ('p=', p, sigp, sep = ''), x = 0.5, y = 0.3,
                  gp=gpar(fontsize=stat.pars$fontsize, col = text.color)) 
        grid.text(paste(missing, 'missing'), x = 0.5, y = 0.1,
                  gp=gpar(fontsize=stat.pars$fontsize, col = 'red')) 
      } else {
         grid.text(paste ('r=', corr, sigcor, sep = ''), x = 0.5, y = 0.7,
                   gp=gpar(fontsize=stat.pars$fontsize, col = text.color)) 
         grid.text(paste(missing, 'missing'), x = 0.5, y = 0.3,
                   gp=gpar(fontsize=stat.pars$fontsize, col = text.color))
      }
    }
  } # End of Stats
...
boxplot.panel <- function(x, y, type, axis.pars, xpos, ypos) {
  ...
  if (horiz) {
    ...
    draw.axis(cont.var, cat.var, axis.pars, xpos, ypos, cat.labels, horiz)
  } else {
    ...
    draw.axis(cat.var, cont.var, axis.pars, xpos, ypos, cat.labels, horiz)
  }
...
diag.panel <- function(x, varname, diag.pars, axis.pars, xpos, ypos) {

  x <- x[!is.na(x)]
  pushViewport(dataViewport(as.numeric(x), as.numeric(x)))
  if (!diag.pars$show.hist) {
    grid.rect()
    grid.text(varname, 0.5, 0.5, gp=gpar(fontsize=diag.pars$fontsize, fontface=2))
  }
  draw.axis(as.numeric(x), as.numeric(x), axis.pars, xpos, ypos, NULL, NULL)
  popViewport(1)
  if (diag.pars$show.hist) {
    if (!is.factor(x)) {
      pushViewport(viewport(xscale=c(min(as.numeric(x), na.rm=TRUE),
...
###########################################################################################
  ...
  for (i in 1:N) {              # i indexes the rows, 1 being the top row
    for (j in 1:N) {            # j indexes the columns, 1 being the left column
      ...
      if (i==j) {
        diag.panel(x[,i], names(x)[i], diag.pars, axis.pars, xpos, ypos)
      } else {
          ...
          if (i<j & upper.pars$conditional=="barcode") {
            if (is.factor(x[,i]))
              barcode(split(x[,j], x[,i])[length(levels(x[,i])):1], 
                            horizontal=TRUE, labelloc=ypos, axisloc=xpos, labelouter=TRUE,
              ...
              if (!is.null(ypos)) ypos <- !ypos
                barcode(split(x[,i], x[,j])[length(levels(x[,j])):1], 
                              horizontal=FALSE, labelloc=xpos, axisloc=ypos, labelouter=TRUE,
                              newpage=FALSE, fontsize=axis.pars$fontsize, buffer=buffer,
                            nint=barcode.pars$nint, ptsize=barcode.pars$ptsize,
                            ptpch=barcode.pars$ptpch, bcspace=barcode.pars$bcspace,
                            use.points=barcode.pars$use.points)
        ...
        if (is.factor(x[,i]) + is.factor(x[,j]) == 0) {
          ...
          scatterplot.panel(x[,j], x[,i], type, scatter.pars, axis.pars, xpos, ypos)
        }
